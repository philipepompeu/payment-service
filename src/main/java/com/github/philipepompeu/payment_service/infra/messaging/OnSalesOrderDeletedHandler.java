...
@Component("onSalesOrderDeleted")
@RequiredArgsConstructor
public class OnSalesOrderDeletedHandler implements PaymentEventHandler {
    ...
    public void handle(PaymentQueueMessageDto message) {
        try {
            ...
            paymentRepository.findByOrderId(entity.getOrderId())
                .switchIfEmpty(Mono.empty())
                .flatMap((found)->{
                    return paymentRepository.delete(found)
                        .doOnSuccess(x -> System.out.println(String.format("Payment[id = %s] deleted.", found.getId())));
                }).subscribe();        
